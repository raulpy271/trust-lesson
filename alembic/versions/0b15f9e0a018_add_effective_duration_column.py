...
def upgrade() -> None:
    op.add_column(
        "lesson", sa.Column("effective_duration_min", sa.Integer(), nullable=True)
    )
    op.alter_column(
        "lesson", "effective_start_date", existing_type=sa.DATE(), nullable=True
    )


def downgrade() -> None:
    op.alter_column(
        "lesson", "effective_start_date", existing_type=sa.DATE(), nullable=False
    )
