...
class CheckAuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        if self.should_be_logged(request):
            logged, token, mapping = await self.verify_token(request)
            request.state.logged = logged
            request.state.token = token
            if logged:
                response = await call_next(request)
                await self.add_logged_headers(response, token, mapping)
            else:
    ...
    async def verify_token(self, request):
        ...
        if authorization:
            ...
            if token:
                ...
                mapping = await hgetall_str(redis, token)
                if (
                    mapping
    ...
    async def add_logged_headers(self, response, token, mapping):
        redis = get_default_client()
        exp_s = await redis.ttl(token)
        exp_ts = int(datetime.now().timestamp() + exp_s)
        if exp_s <= settings.TOKEN_REGENERATE:
            token, exp_ts = await generate_new_token(token, mapping)
